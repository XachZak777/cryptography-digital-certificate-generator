"""
Advanced Encryption Standard (AES) Implementation
A block cipher that operates on 128-bit blocks using keys of 128, 192, or 256 bits.

This implementation provides AES-128, AES-192, and AES-256 encryption/decryption
using the Rijndael algorithm as specified in FIPS PUB 197.
"""

# AES S-Box (Substitution Box) - used in SubBytes transformation
S_BOX = [
    
]

# Inverse S-Box for decryption
INV_S_BOX = [
    
]

# Round constants for key expansion
RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]


def sub_bytes(state: list) -> list:
    """
    SubBytes transformation: substitutes each byte in the state using S-Box.
    
    Args:
        state: 4x4 state matrix (list of 4 lists, each with 4 bytes)
    
    Returns:
        Transformed state matrix
    """
    for i in range(4):
        for j in range(4):
            state[i][j] = S_BOX[state[i][j]]
    return state


def inv_sub_bytes(state: list) -> list:
    """
    Inverse SubBytes transformation for decryption.
    
    Args:
        state: 4x4 state matrix
    
    Returns:
        Transformed state matrix
    """
    for i in range(4):
        for j in range(4):
            state[i][j] = INV_S_BOX[state[i][j]]
    return state


def shift_rows(state: list) -> list:
    """
    ShiftRows transformation: cyclically shifts rows of the state matrix.
    Row 0: no shift, Row 1: shift left by 1, Row 2: shift left by 2, Row 3: shift left by 3.
    
    Args:
        state: 4x4 state matrix
    
    Returns:
        Transformed state matrix
    """
    # Row 0: no shift
    # Row 1: shift left by 1
    state[1][0], state[1][1], state[1][2], state[1][3] = state[1][1], state[1][2], state[1][3], state[1][0]
    # Row 2: shift left by 2
    state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[2][3], state[2][0], state[2][1]
    # Row 3: shift left by 3
    state[3][0], state[3][1], state[3][2], state[3][3] = state[3][3], state[3][0], state[3][1], state[3][2]
    return state


def inv_shift_rows(state: list) -> list:
    """
    Inverse ShiftRows transformation for decryption.
    
    Args:
        state: 4x4 state matrix
    
    Returns:
        Transformed state matrix
    """
    # Row 0: no shift
    # Row 1: shift right by 1
    state[1][0], state[1][1], state[1][2], state[1][3] = state[1][3], state[1][0], state[1][1], state[1][2]
    # Row 2: shift right by 2
    state[2][0], state[2][1], state[2][2], state[2][3] = state[2][2], state[2][3], state[2][0], state[2][1]
    # Row 3: shift right by 3
    state[3][0], state[3][1], state[3][2], state[3][3] = state[3][1], state[3][2], state[3][3], state[3][0]
    return state


def gmul(a: int, b: int) -> int:
    """
    Galois Field (GF(2^8)) multiplication.
    Used in MixColumns transformation.
    
    Args:
        a: First byte
        b: Second byte
    
    Returns:
        Product in GF(2^8)
    """
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        if hi_bit_set:
            a ^= 0x1b  # Reduction polynomial for GF(2^8)
        b >>= 1
    return p & 0xff


def mix_columns(state: list) -> list:
    """
    MixColumns transformation: multiplies each column by a fixed matrix.
    
    Args:
        state: 4x4 state matrix
    
    Returns:
        Transformed state matrix
    """
    new_state = [[0 for _ in range(4)] for _ in range(4)]
    for c in range(4):
        new_state[0][c] = gmul(0x02, state[0][c]) ^ gmul(0x03, state[1][c]) ^ state[2][c] ^ state[3][c]
        new_state[1][c] = state[0][c] ^ gmul(0x02, state[1][c]) ^ gmul(0x03, state[2][c]) ^ state[3][c]
        new_state[2][c] = state[0][c] ^ state[1][c] ^ gmul(0x02, state[2][c]) ^ gmul(0x03, state[3][c])
        new_state[3][c] = gmul(0x03, state[0][c]) ^ state[1][c] ^ state[2][c] ^ gmul(0x02, state[3][c])
    return new_state


def inv_mix_columns(state: list) -> list:
    """
    Inverse MixColumns transformation for decryption.
    
    Args:
        state: 4x4 state matrix
    
    Returns:
        Transformed state matrix
    """
    new_state = [[0 for _ in range(4)] for _ in range(4)]
    for c in range(4):
        new_state[0][c] = gmul(0x0e, state[0][c]) ^ gmul(0x0b, state[1][c]) ^ gmul(0x0d, state[2][c]) ^ gmul(0x09, state[3][c])
        new_state[1][c] = gmul(0x09, state[0][c]) ^ gmul(0x0e, state[1][c]) ^ gmul(0x0b, state[2][c]) ^ gmul(0x0d, state[3][c])
        new_state[2][c] = gmul(0x0d, state[0][c]) ^ gmul(0x09, state[1][c]) ^ gmul(0x0e, state[2][c]) ^ gmul(0x0b, state[3][c])
        new_state[3][c] = gmul(0x0b, state[0][c]) ^ gmul(0x0d, state[1][c]) ^ gmul(0x09, state[2][c]) ^ gmul(0x0e, state[3][c])
    return new_state


def add_round_key(state: list, round_key: list) -> list:
    """
    AddRoundKey transformation: XORs the state with the round key.
    
    Args:
        state: 4x4 state matrix
        round_key: 4x4 round key matrix
    
    Returns:
        Transformed state matrix
    """
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]
    return state


def key_expansion(key: bytes) -> list:
    """
    Key expansion: generates round keys from the original key.
    Supports AES-128 (16 bytes), AES-192 (24 bytes), and AES-256 (32 bytes).
    
    Args:
        key: Encryption key (16, 24, or 32 bytes)
    
    Returns:
        List of round keys (each round key is a 4x4 matrix)
    """
    key_size = len(key)
    if key_size not in [16, 24, 32]:
        raise ValueError(f"Invalid key size: {key_size}. Must be 16, 24, or 32 bytes")
    
    # Determine number of rounds and key words
    if key_size == 16:  # AES-128
        nk, nr = 4, 10
    elif key_size == 24:  # AES-192
        nk, nr = 6, 12
    else:  # AES-256
        nk, nr = 8, 14
    
    # Initialize key schedule
    w = []
    for i in range(nk):
        w.append([key[4*i], key[4*i+1], key[4*i+2], key[4*i+3]])
    
    # Expand key
    for i in range(nk, 4 * (nr + 1)):
        temp = w[i-1].copy()
        if i % nk == 0:
            # RotWord
            temp = [temp[1], temp[2], temp[3], temp[0]]
            # SubWord
            temp = [S_BOX[b] for b in temp]
            # XOR with Rcon
            temp[0] ^= RCON[(i // nk) - 1]
        elif nk > 6 and i % nk == 4:
            # SubWord for AES-256
            temp = [S_BOX[b] for b in temp]
        
        # XOR with previous word
        w.append([w[i-nk][j] ^ temp[j] for j in range(4)])
    
    # Convert to round keys (4x4 matrices)
    round_keys = []
    for i in range(nr + 1):
        round_key = [[0 for _ in range(4)] for _ in range(4)]
        for j in range(4):
            for k in range(4):
                round_key[j][k] = w[i*4 + j][k]
        round_keys.append(round_key)
    
    return round_keys


def bytes_to_state(data: bytes) -> list:
    """
    Convert 16 bytes to a 4x4 state matrix (column-major order).
    
    Args:
        data: 16 bytes of data
    
    Returns:
        4x4 state matrix
    """
    if len(data) != 16:
        raise ValueError("Data must be exactly 16 bytes")
    state = [[0 for _ in range(4)] for _ in range(4)]
    for i in range(4):
        for j in range(4):
            state[j][i] = data[i*4 + j]
    return state


def state_to_bytes(state: list) -> bytes:
    """
    Convert a 4x4 state matrix to 16 bytes (column-major order).
    
    Args:
        state: 4x4 state matrix
    
    Returns:
        16 bytes of data
    """
    data = bytearray(16)
    for i in range(4):
        for j in range(4):
            data[i*4 + j] = state[j][i]
    return bytes(data)


def aes_encrypt_block(plaintext: bytes, key: bytes) -> bytes:
    """
    Encrypt a single 16-byte block using AES.
    
    Args:
        plaintext: 16 bytes of plaintext
        key: Encryption key (16, 24, or 32 bytes)
    
    Returns:
        16 bytes of ciphertext
    """
    if len(plaintext) != 16:
        raise ValueError("Plaintext must be exactly 16 bytes")
    
    # Expand key
    round_keys = key_expansion(key)
    nr = len(round_keys) - 1
    
    # Convert to state matrix
    state = bytes_to_state(plaintext)
    
    # Initial round: AddRoundKey only
    state = add_round_key(state, round_keys[0])
    
    # Main rounds
    for round_num in range(1, nr):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, round_keys[round_num])
    
    # Final round (no MixColumns)
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, round_keys[nr])
    
    # Convert back to bytes
    return state_to_bytes(state)


def aes_decrypt_block(ciphertext: bytes, key: bytes) -> bytes:
    """
    Decrypt a single 16-byte block using AES.
    
    Args:
        ciphertext: 16 bytes of ciphertext
        key: Decryption key (16, 24, or 32 bytes)
    
    Returns:
        16 bytes of plaintext
    """
    if len(ciphertext) != 16:
        raise ValueError("Ciphertext must be exactly 16 bytes")
    
    # Expand key
    round_keys = key_expansion(key)
    nr = len(round_keys) - 1
    
    # Convert to state matrix
    state = bytes_to_state(ciphertext)
    
    # Initial round: AddRoundKey only
    state = add_round_key(state, round_keys[nr])
    
    # Main rounds
    for round_num in range(nr - 1, 0, -1):
        state = inv_shift_rows(state)
        state = inv_sub_bytes(state)
        state = add_round_key(state, round_keys[round_num])
        state = inv_mix_columns(state)
    
    # Final round (no InvMixColumns)
    state = inv_shift_rows(state)
    state = inv_sub_bytes(state)
    state = add_round_key(state, round_keys[0])
    
    # Convert back to bytes
    return state_to_bytes(state)


def aes_cbc_encrypt(plaintext: bytes, key: bytes, iv: bytes) -> bytes:
    """
    Encrypt data using AES in CBC (Cipher Block Chaining) mode.
    
    Args:
        plaintext: Data to encrypt (must be multiple of 16 bytes)
        key: Encryption key (16, 24, or 32 bytes)
        iv: Initialization vector (16 bytes)
    
    Returns:
        Ciphertext
    """
    if len(iv) != 16:
        raise ValueError("IV must be exactly 16 bytes")
    if len(plaintext) % 16 != 0:
        raise ValueError("Plaintext must be multiple of 16 bytes (use padding)")
    
    ciphertext = bytearray()
    previous_block = iv
    
    for i in range(0, len(plaintext), 16):
        block = plaintext[i:i+16]
        # XOR with previous ciphertext block (or IV for first block)
        xored_block = bytes(a ^ b for a, b in zip(block, previous_block))
        # Encrypt
        encrypted_block = aes_encrypt_block(xored_block, key)
        ciphertext.extend(encrypted_block)
        previous_block = encrypted_block
    
    return bytes(ciphertext)


def aes_cbc_decrypt(ciphertext: bytes, key: bytes, iv: bytes) -> bytes:
    """
    Decrypt data using AES in CBC mode.
    
    Args:
        ciphertext: Data to decrypt (must be multiple of 16 bytes)
        key: Decryption key (16, 24, or 32 bytes)
        iv: Initialization vector (16 bytes)
    
    Returns:
        Plaintext
    """
    if len(iv) != 16:
        raise ValueError("IV must be exactly 16 bytes")
    if len(ciphertext) % 16 != 0:
        raise ValueError("Ciphertext must be multiple of 16 bytes")
    
    plaintext = bytearray()
    previous_block = iv
    
    for i in range(0, len(ciphertext), 16):
        block = ciphertext[i:i+16]
        # Decrypt
        decrypted_block = aes_decrypt_block(block, key)
        # XOR with previous ciphertext block (or IV for first block)
        xored_block = bytes(a ^ b for a, b in zip(decrypted_block, previous_block))
        plaintext.extend(xored_block)
        previous_block = block
    
    return bytes(plaintext)

